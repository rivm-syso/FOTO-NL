...
```{r}
#list all packages
list.of.packages <- c("gridExtra", "readr", "dplyr", "sf", "ggplot2", "scales", "lubridate", "tidyr", "tibble", "forcats")
...
scientific4_scalar <- function(x) {
  if (length(x) == 0 || is.na(x)) return(NA_character_)
  sprintf("%.4e", as.numeric(x))
}

write_selected_scientific_csv <- function(df, path, cols) {
  out <- df
  cols <- intersect(cols, names(out))
  for (col in cols) {
    out[[col]] <- vapply(out[[col]], scientific4_scalar, character(1))
  }
  write_csv(out, path, na = "")
}

# Normalize column aliases from the current stage-1/stage-2 output contract.
if (!"Parametercode" %in% names(AllData) && "AquoCode" %in% names(AllData)) {
  AllData$Parametercode <- AllData$AquoCode
}
if (!"LocationCode" %in% names(AllData) && "Locationcode" %in% names(AllData)) {
  AllData$LocationCode <- AllData$Locationcode
}
...
legend_levels <- c("All", all_sources)
...
Substance_df_fn <- function(substance, plottype) {
  ...
  range_text <- paste("Range:", scientific4_scalar(min_value), "-", scientific4_scalar(max_value))
  ...
  ecdf_df <- bind_rows(loc_all_df, subdata %>% mutate(SampleID = "All")) %>%
    mutate(SampleID = factor(SampleID, levels = legend_levels)) %>%
    arrange(desc(SampleID == "All"))
  ...
  ggplot(ecdf_df, aes(x = MeasuredValue, color = SampleID)) +
    ...
    scale_color_manual(values = global_colors, limits = legend_levels, drop = FALSE) +
...
create_overview_table <- function(substance_list) {
  overview <- lapply(substance_list, function(substance) {
    ...
    data.frame(
      Substance = substance,
      Lower_Whisker = scientific4_scalar(lower_whisker),
      Upper_Whisker = scientific4_scalar(upper_whisker),
...
write_selected_scientific_csv(overview_table, "summary_stats/substance_iqr_table.csv", c("Lower_Whisker", "Upper_Whisker"))
...
write_selected_scientific_csv(overview_table, "summary_stats/metal_iqr_table.csv", c("Lower_Whisker", "Upper_Whisker"))
```
...
write_selected_scientific_csv(overview_table, "summary_stats/pesticide_iqr_table.csv", c("Lower_Whisker", "Upper_Whisker"))
```
