---
title: "script_2_spatial_analysis_raw"
...
coords <- unique(AllData[,c("SampleID","Meetpuntcode","X", "Y")])
...
```{r}
coords_year <- unique(AllData[,c("SampleID","Meetpuntcode","X", "Y", "year")])
...
coordsNL <- unique(AllDataNL[,c("DataSource","Meetpuntcode","X", "Y")])
...
AllDataNL$`NA` <- NULL
AllDataNL <- AllDataNL %>% rename(LocationCode = Meetpuntcode)
...
fwrite(AllDataNL, "output/foto_nl_dataset_raw.csv", sep = ";")
```
